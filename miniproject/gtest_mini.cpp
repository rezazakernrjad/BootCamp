...
void TestBokShop::SetUp(){
    ...
    cout << "SetUp:: " << bookshop_->getBook()[1].auth << endl;
}
...
TEST_F(TestBokShop, SearchBook){
    ...
    title_author_test[1] = "a_3";
    int s = bookshop_->SearchBook(bookshop_);
    EXPECT_EQ(s,2); // t_1, a_1 is the first book in the book_[] with index=0;
...
TEST_F(TestBokShop, EditBook){
    title_author_test[0] = "t_5";
    title_author_test[1] = "a_5";
    int s = bookshop_->SearchBook(bookshop_);
    EXPECT_EQ(s,4);
